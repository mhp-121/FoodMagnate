...
class Company:
    ...
    def AlterFuelCostPerUnit(self, Change):
        FuelCostFloor = 0
        if (self._FuelCostPerUnit + Change) <= FuelCostFloor:
    ...
    def GetDetails(self):
        ...
        Details += "Current balance: " + Simulation.NumToCurrency(self._Balance) + "\nAverage cost per meal: " + Simulation.NumToCurrency(
            self._AvgCostPerMeal) + "\n"
        Details += "Average price per meal: " + Simulation.NumToCurrency(self._AvgPricePerMeal) + "\nDaily costs: " + Simulation.NumToCurrency(
            self._DailyCosts) + "\n"
        Details += "Delivery costs: " + Simulation.NumToCurrency(self.CalculateDeliveryCost()) + "\nReputation: " + str(
    ...
    def ProcessDayEnd(self):
        ...
        Details += "Daily costs for company: " + Simulation.NumToCurrency(self._DailyCosts
            ) + "\nCost for delivering produce to outlets: " + Simulation.NumToCurrency(DeliveryCosts) + "\n"
        for Current in range(0, len(self._Outlets)):
            ...
            Details += "Outlet " + str(Current + 1) + " profit/loss: " + Simulation.NumToCurrency(ProfitLossFromThisOutlet) + "\n"
            ProfitLossFromOutlets += ProfitLossFromThisOutlet
        Details += "Previous balance for company: " + Simulation.NumToCurrency(self._Balance) + "\n"
        self._Balance += ProfitLossFromOutlets - self._DailyCosts - DeliveryCosts
        Details += "New balance for company: " + Simulation.NumToCurrency(self._Balance)
        return Details
...
class Simulation:
    ...
    def __ProcessCostOfFuelChangeEvent(self):
        ...
        if UpOrDown == 0:
            print("The cost of fuel has gone up by " + self.NumToCurrency(FuelCostChange) + " for " + self._Companies[
                CompanyNo].GetName())
        else:
            print("The cost of fuel has gone down by " + self.NumToCurrency(FuelCostChange) + " for " + self._Companies[
                CompanyNo].GetName())
            FuelCostChange *= -1
    ...
    def __ProcessCostChangeEvent(self):
        ...
        if CostToChange == 0:
            ...
            if UpOrDown == 0:
                print("The daily costs for " + self._Companies[CompanyNo].GetName() + " have gone up by " + self.NumToCurrency(
                    AmountOfChange))
            else:
                print("The daily costs for " + self._Companies[CompanyNo].GetName() + " have gone down by " + self.NumToCurrency(
                    AmountOfChange))
                AmountOfChange *= -1
            ...
            if UpOrDown == 0:
                print(
                    "The average cost of a meal for " + self._Companies[CompanyNo].GetName() + " has gone up by " + self.NumToCurrency(
                        AmountOfChange))
            else:
                print("The average cost of a meal for " + self._Companies[
                    CompanyNo].GetName() + " has gone down by " + self.NumToCurrency(AmountOfChange))
                AmountOfChange *= -1
